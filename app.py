...
def create_cloze_word(word):
    ...
    if not indices: return word # 防止 'at' 這種
...
def run_listening_mode():
    ...
    if st.session_state.game_state == "START":
        ...
        play_audio_js(q['en'], key_suffix=f"lis_{st.session_state.current_idx}")
...
def run_cloze_mode():
    ...
    if st.session_state.game_state == "START":
        ...
        # 產生挖空單字 (只在第一次載入題目時產生，避免重刷變動)
        if 'cloze_word' not in st.session_state or not st.session_state.options:
            st.session_state.cloze_word = create_cloze_word(q['en'])
            
        st.markdown(f"## {st.session_state.cloze_word}")
        
        # 輔助發音 (可選)
        play_audio_js(q['en'], key_suffix=f"cloze_{st.session_state.current_idx}")
